...
namespace Microsoft.CorrelationVector
{
    ...
    public sealed class CorrelationVectorV1 : CorrelationVector
    {
        ...
        public new static CorrelationVectorV1 Extend(string correlationVector)
        {
            ...
            CorrelationVectorVersion version = CorrelationVectorV1.InferVersion(
                correlationVector, CorrelationVectorV1.ValidateCorrelationVectorDuringCreation);

            if (CorrelationVectorV1.ValidateCorrelationVectorDuringCreation)
            {
                CorrelationVectorV1.Validate(correlationVector, version);
            }

            if (CorrelationVectorV1.IsOversized(correlationVector, 0, version))
            {
                return CorrelationVectorV1.Parse(correlationVector + CorrelationVectorV1.TerminationSign);
            }

            return new CorrelationVectorV1(correlationVector, 0, version, false);
        }
        ...
        public new static CorrelationVectorV1 Spin(string correlationVector)
        {
            ...
            return CorrelationVectorV1.Spin(correlationVector, defaultParameters);
        }
        ...
        public new static CorrelationVectorV1 Spin(string correlationVector, SpinParameters parameters)
        {
            ...
            if (CorrelationVectorV1.IsOversized(baseVector, 0, version))
            {
                return CorrelationVectorV1.Parse(correlationVector + CorrelationVectorV1.TerminationSign);
            }

            return new CorrelationVectorV1(baseVector, 0, version, false);
        }
        ...
        public new static CorrelationVectorV1 Parse(string correlationVector)
        {
            if (!string.IsNullOrEmpty(correlationVector))
            {
                ...
                if (p > 0)
                {
                    ...
                    {
                        return new CorrelationVectorV1(correlationVector.Substring(0, p), extension, CorrelationVectorV1.InferVersion(correlationVector, false), immutable);
                    }
                }
            }
        ...
        /// <summary>
        /// Initializes a new instance of the <see cref="CorrelationVectorV1"/> class. This
        ...
        public CorrelationVectorV1(CorrelationVectorVersion version)
            : this(CorrelationVectorV1.GetUniqueValue(version), 0, version, false)
        {
        }
        ...
        public override string Increment()
        {
            ...
            do
            {
                ...
                next = snapshot + 1;
                if (CorrelationVectorV1.IsOversized(this.BaseVector, next, this.Version))
                {
                    this.immutable = true;
                    return this.Value;
                }
        ...
        private CorrelationVectorV1(string baseVector, int extension, CorrelationVectorVersion version, bool immutable)
        {
            this.BaseVector = baseVector;
            this.extension = extension;
            this.Version = version;
            this.immutable = immutable || CorrelationVectorV1.IsOversized(baseVector, extension, version);
        }
        ...
        private static bool IsOversized(string baseVector, int extension, CorrelationVectorVersion version)
        {
            if (!string.IsNullOrEmpty(baseVector))
            {
                ...
                return ((version == CorrelationVectorVersion.V1 &&
                      size > CorrelationVectorV1.MaxVectorLength) ||
                     (version == CorrelationVectorVersion.V2 &&
                      size > CorrelationVectorV1.MaxVectorLengthV2));
            }
            return false;
        }
        ...
        private static void Validate(string correlationVector, CorrelationVectorVersion version)
        {
            ...
            {
                maxVectorLength = CorrelationVectorV1.MaxVectorLengthV2;
                baseLength = CorrelationVectorV1.BaseLengthV2;
            }
            else
            {
                throw new ArgumentException(string.Format(CultureInfo.CurrentCulture, "Unsupported correlation vector version: {0}", version));
